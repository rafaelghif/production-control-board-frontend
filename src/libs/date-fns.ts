...
export const plus1HourTime = (stringTime: string) => {
	const stringPlusTime = (parseInt(stringTime.split(":")[0]) + 1)
		.toString()
		.padStart(2, "0");
...
export const getCurrentIsoDateIonic = () => {
	const date = format(parseISO(new Date().toISOString()), "yyyy-MM-dd", {
		locale: id,
	});
	const time = format(parseISO(new Date().toISOString()), "HH:mm", {
		locale: id,
	});
