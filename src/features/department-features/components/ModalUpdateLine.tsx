...
const ModalUpdateLine: React.FC<ModalUpdateLineProps> = ({ isOpen, data, onDidDismiss }) => {
    const [formData, setFormData] = useState<UpdateLineType>({});
    const { mutate } = useUpdateLine();

    const handleInput = (key: keyof UpdateLineType, value: string | boolean) => {
    ...
    useEffect(() => {
        setFormData((prevState) => ({ ...prevState, id: data?.id, name: data?.name, inActive: data?.inActive, DepartmentId: data?.DepartmentId }));
    }, [data]);
