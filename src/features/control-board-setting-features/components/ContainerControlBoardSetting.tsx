...
const TableControlBoardSetting = lazy(() => import("./TableControlBoardSetting"));

const ContainerControlBoardSetting: React.FC = () => {
    const { department } = useDepartmentStore();
    const { user } = useUserStore();

    const { data, isLoading, refetch } = user.role !== "Basic" ? useQueryControlBoard() : useQueryControlBoardSettingByDepartment(department.id);
    const [selectValue, setSelectValue] = useState<ControlBoardSettingInterface>();
    ...
    return (
        <>
                        ...
                        <Card title="Data Control Board Setting">
                            {isLoading ? (
                                ...
                                <Suspense fallback={<IonSpinner name="crescent" />}>
                                    <TableControlBoardSetting data={data} handleClickBtnEdit={(data) => handleClickBtnEdit(data)} />
                                </Suspense>
                            )}
                        </Card>
                    </IonCol>
            ...
            <ModalUpdateControlBoardSetting data={selectValue} isOpen={isOpenModalUpdate} onDidDismiss={() => setIsOpenModalUpdate(false)} />
        </>
    );
}
